...
이 저장소는 LendingClub P2P 대출에서 각 대출의 “투자 매력도”를 개별 샤프비율로 정의하고, 승인 임계값 $$\tau$$를 조정해 $$SR_i \ge \tau$$ 인 대출만 승인(투자)했을 때의 포트폴리오 샤프비율을 최대화하는 파이프라인입니다. 즉, 먼저 대출별 부도확률 $$p_i$$를 예측한 뒤 대출별 샤프비율 $$SR_i$$를 계산하고, $$\tau$$에 따라 내생적으로 달라지는 승인 집합으로 포트폴리오를 구성하여 검증 데이터에서 최적의 $$\tau$$를 선택합니다.

개별 대출의 평균수익률은 기대수익률로 정의하며, 부도확률과 정상/부도 시 실현수익률을 결합해 $$E[r_i] = (1-p_i)\,r_{\mathrm{normal},d(i)} + p_i\,r_{\mathrm{default},d(i)}$$ 로 계산합니다. 여기서 $$r_{\mathrm{normal},d}$$ 와 $$r_{\mathrm{default},d}$$ 는 예측 부도확률을 십분위로 나눈 구간 $$d(i)$$ 별로 추정한 정상/부도 조건부 평균 실현수익률로, 위험 수준에 따라 회수율이 달라지는 이질성을 반영합니다. 변동성은 개별 대출에서 직접 추정하기 어렵기 때문에(대출당 실현수익률 1개), 기대수익률을 여러 구간으로 나눈 뒤 각 구간 내 실현수익률의 횡단면 분산을 위험 대리변수로 사용하여 $$SR_i = E[r_i] / \sigma_{d(i)}$$ 를 계산하고, $$\tau$$ 변화에 따른 평균수익–변동성의 동시 변화를 반영합니다.
