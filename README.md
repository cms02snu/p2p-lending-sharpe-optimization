...
이 저장소는 LendingClub P2P 대출 데이터로, “승인 임계값(tau)”을 조정해 승인(투자)되는 대출 집합이 내생적으로 바뀌는 상황에서
포트폴리오 샤프비율(Sharpe ratio)을 최대화하는 파이프라인을 구현합니다. 대출별 부도확률 p를 예측(XGBoost+Optuna)하고,
각 tau에 대해 승인된 대출로 포트폴리오를 구성한 뒤, 검증 데이터에서 샤프비율이 최대가 되는 tau를 선택합니다.

기대수익률은 (1) 상수 부도수익률 방식과 (2) 위험수준 이질성을 반영한 “부도확률 십분위 조건부 수익률” 방식(v4)을 지원합니다.
개별 대출은 실현수익률이 1개뿐이라 위험을 직접 추정하기 어려우므로, 기대수익률 구간 내 횡단면 분산을 위험 대리변수로 사용해
tau 변화에 따른 평균수익–변동성의 동시 변화를 포착합니다.
